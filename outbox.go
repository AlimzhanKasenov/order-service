...
// сохранённое в PostgreSQL до отправки в Kafka.
...
// newEventID генерирует UUID v4 без внешних зависимостей.
...
// insertOutboxEventTx сохраняет событие в Outbox.
//
// Важно: функция получает уже открытую транзакцию.
// Поэтому изменение бизнес-данных и создание события
// коммитятся одной PostgreSQL-транзакцией.
...
// reserveInboxEventTx регистрирует полученное событие.
//
// true означает, что событие получено впервые.
// false означает, что event_id уже существует и
// повторное событие не должно обрабатываться.
//
// Inbox-запись и бизнес-изменение должны выполняться
...
// runOutboxPublisher запускает постоянный relay:
//
// PostgreSQL Outbox -> Kafka.
//
// Событие удалять из таблицы не нужно.
// После успешной отправки оно получает статус PUBLISHED.
...
// publishOneOutboxEvent отправляет одно PENDING-событие.
//
// FOR UPDATE SKIP LOCKED позволяет безопасно запускать
// несколько экземпляров publisher одновременно:
// одна Outbox-запись не будет взята двумя worker'ами
